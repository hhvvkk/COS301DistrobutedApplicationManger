...
Our system can interact with Subversion (SVN), which is where the clients store the various builds of their applications. We will also be connecting to a MySql database where the information concerning configurations on slave pc's will be stored. XML is also an option given by the client for storing configuration information.
\begin{itemize}
